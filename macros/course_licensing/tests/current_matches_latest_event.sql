{#
    Cross-checks a course_licensing _current model against an independently
    ...
    native unit-test comparison (confirmed empirically — see
    mv_course_licensing_course_enrollment_allowed_latest_state.sql).
    ...
    not a number, in every course_licensing raw sink table.
#}
{% test course_licensing_current_matches_latest_event(
    model, source_name, source_table
) %}
        ...
        ranked_source as (
            select
                ...
                row_number() over (
                    ...
                    order by ({{ course_licensing_latest_state_order() }}) desc
                ) as event_rank
